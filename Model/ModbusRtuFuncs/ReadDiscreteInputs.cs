...
{
    public class ReadDiscreteInputs : DefaultModel
    {
        public ReadDiscreteInputs() : base("Чтение значений из нескольких дискретных входов (Read Discrete Inputs)", 0x02)
        {
            IsStartAddress = true;
        ...
        public override void SendMessage(DeviceModel device, ModbusSerialMaster master, ref string message, bool isRTU = true)
        {
            ...
            try
            {
                values = master.ReadInputs(1, StartAddress, NumberOfPoint);
            }
            catch
            {
